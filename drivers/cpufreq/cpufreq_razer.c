...
#include <trace/events/cpufreq_razer.h>
...
static int razer_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void razer_powersave_bias_init_cpu(int cpu)
{
...
static void razer_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		razer_powersave_bias_init_cpu(i);
	}
}
...
skip_this_cpu:
		...
		for_each_online_cpu(cpu) {
			...
			if (dbs_info->cur_policy) {
                                /* cpu using razer, cancel dbs timer */
				dbs_timer_exit(dbs_info);

                                mutex_lock(&dbs_info->timer_mutex);
				razer_powersave_bias_setspeed(
					dbs_info->cur_policy,
					NULL,
					input);
				mutex_unlock(&dbs_info->timer_mutex);
...
static void dbs_freq_increase(struct cpufreq_policy *p, unsigned load, unsigned int freq)
{
	if (dbs_tuners_ins.powersave_bias)
		...
		trace_cpufreq_razer_already (p->cpu, load, p->cur, p->cur, p->cur);
		return;
	}

	trace_cpufreq_razer_target (p->cpu, load, p->cur, p->cur, freq);

	__cpufreq_driver_target(p, freq, (dbs_tuners_ins.powersave_bias || freq < p->max) ?
			CPUFREQ_RELATION_L : CPUFREQ_RELATION_H);

	trace_cpufreq_razer_up (p->cpu, freq, p->cur);
}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	{
		trace_cpufreq_razer_already (policy->cpu, cur_load, policy->cur, policy->cur, policy->cur);
		return;
	}
	...
	if (max_load_freq <
		...
		trace_cpufreq_razer_target (policy->cpu, cur_load, policy->cur, policy->cur, freq_next);
		__cpufreq_driver_target(policy, freq_next,
			CPUFREQ_RELATION_L);
		trace_cpufreq_razer_down (policy->cpu, freq_next, policy->cur);
	}
}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		razer_powersave_bias_init_cpu(cpu);
		if (dbs_enable == 1) {
		...
		if (!razer_powersave_bias_setspeed(
					this_dbs_info->cur_policy,
					NULL,
					dbs_tuners_ins.powersave_bias))
			dbs_timer_init(this_dbs_info);
		trace_cpufreq_razer_target (cpu, 0, 0, 0, 0);
		break;
...
MODULE_DESCRIPTION("'cpufreq_razer' - multiphase dynamic cpufreq governor");
